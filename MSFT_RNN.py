...
msft_df['Open'].plot(figsize=(16, 6))
msft_df.rolling(100).mean()['Open'].plot()


# %%

# %%
training_df = msft_df['Open']
...
# %%
df_total = pd.concat((msft_df['Open'], df_test['Open']), axis = 0)
...
df_test = msft_data_copy.tail(40)

# Remove the test data from msft_data for further partitioning
msft_data_copy = msft_data_copy[:-40]

# Calculate the training data size (80% of the remaining data)
train_size = int(0.8 * len(msft_data_copy))

# Split the data into training and validation sets
train_data = msft_data_copy[:train_size]
val_data = msft_data_copy[train_size:]
